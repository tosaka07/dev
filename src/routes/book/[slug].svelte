...
<div class="w-full mb-8 bg-center bg-cover" style="background-image: url({data.api.summary.cover || '/noimage.jpg'});">
    ...
    <div class="article-container">
      <div class="flex flex-col md:flex-row items-center">
        ...
        <div class="flex flex-col">
          ...
          <h1 class="text-3xl md:text-4xl font-bold text-white">{data.api.summary.title}</h1>
          <p class="text-white mb-2">{data.api.summary.author}</p>
          {#if data.api.onix.CollateralDetail.TextContent[0].Text != undefined}
            <p class="text-sm text-white">{data.api.onix.CollateralDetail.TextContent[0].Text}</p>
          {/if}
        </div>
      </div>
    </div>
...
<div class="article-container">
  <h2 class="text-base text-gray-600">{data.post.localizedReadAt} に読みました</h2>
</div>
